...
public class D20108 extends D201 {
    ...
    public void actuatorSetOuput(EnJConnection connection, byte[] deviceAddress, boolean command) {
        ...
        super.actuatorSetOutput(connection, deviceAddress, D201DimMode.SWITCH_TO_NEW_OUTPUT_VALUE.getCode(), ALL_OUTPUT_CHANNEL, command ? ON_BYTE : OFF_BYTE);
    }
    ...
    public void actuatorSetOuput(EnJConnection connection, byte[] deviceAddress, int dimValue, D201DimMode dimMode) {
        ...
        super.actuatorSetOutput(connection, deviceAddress, dimMode.getCode(), ALL_OUTPUT_CHANNEL, (byte) dimValue);
    ...
    public void actuatorSetLocal(EnJConnection connection, byte[] deviceAddress, int channelId, EEPAttribute<?>[] attributes, D201DimTime dimTime1, D201DimTime dimTime2, D201DimTime dimTime3) {
        ...
        super.actuatorSetLocal(connection, deviceAddress, (byte) channelId, localControl, overCurrentShutDown, resetOverCurrentShutDown, userInterfaceIndication, powerFailure, defaultState, dimTime1, dimTime2, dimTime3);
    }
    ...
    public void actuatorSetMeasurement(EnJConnection connection, byte[] deviceAddress, boolean autoReportMesurement, boolean signalResetMeasurement, boolean powerMode, int channelId, int measurementDeltaToBeReported, D201UnitOfMeasure unitOfMeasure, int maximumTimeBetweenActuatorMessages, int minimumTimeBetweenActuatorMessages) {
        if (maximumTimeBetweenActuatorMessages >= 0 && minimumTimeBetweenActuatorMessages >= 0) {
            ...
            super.actuatorSetMeasurement(connection, deviceAddress, reportMeasurementAsByte, signalResetMeasurementAsByte, powerModeAsByte, (byte) channelId, measurementDeltaLSB, unitOfMeasure.getCode(), measurementDeltaMSB, maximumTimeAsByte, minimumTimeAsByte);
        }
        else {
