 ...
 */
package com._4ng.enocean.enj.eep.eep26.D2.D201;
...
public abstract class D201 extends EEP {
     ...
     *
     * @param connection  The {@EnJConnection} link to be used for
     *                    sending the command, packet encapsulation will be performed at
     *                    such level.
     * @param dimValue    The required dimming value (low-level, in byte).
     * @param ioChannel   The output channel to be controlled (each device may have more
     ...
     *                                 sending the command, packet encapsulation will be performed at
     *                                 such level.
     * @param deviceAddress            The low level address of the device to which the configuration
     *                                 command must be sent.
     * @param channelId                The id of the channel to be configured.
     * @param localControl             Enables / disables the local control.
     * @param overCurrentShutDown      Configures the behavior of the device in case of over-current
     *                                 shutdown.
     * @param resetOverCurrentShutDown Configures the device behavior when resetting after an
     *                                 over-current shutdown.
     * @param userInterfaceIndication  Sets the user interface mode, from day to night.
     * @param powerFailure             Sets the device behavior upon a power failure event.
     * @param defaultState             Sets the default device state, to be restored when the device
     *                                 powers up.
     * @param dimTime1                 The dim timer 1.
    ...
    public void actuatorSetLocal(EnJConnection connection, byte[] deviceAddress, byte channelId, byte localControl, byte overCurrentShutDown, byte resetOverCurrentShutDown, byte userInterfaceIndication, byte powerFailure, byte defaultState, D201DimTime dimTime1, D201DimTime dimTime2, D201DimTime dimTime3) {
        ...
        // bit 0-4 -> dim timer 1
        dataByte[4] = (byte) ((userInterfaceIndication << 7) + (powerFailure << 6) + (defaultState << 4) + dimTime1.getCode());
     ...
     *                      sending the command, packet encapsulation will be performed at
     *                      such level.
     * @param deviceAddress The low level address of the device to which the status query
     *                      command must be sent.
     ...
     * @param resetMeasurement    0b0 not active, 0b1 trigger signal.
     * @param measurementMode     0b0 energy measurement, 0b1 power measurement.
     * @param channelId           Output channel from 0x00 to 0x1D.
     * @param measurementDeltaLSB Measurement delta to be reported (LSB) 0-4095.
     * @param unit                Unit of measure, 0x00 Energy (Ws), 0x01 Energy (Wh), 0x02
     *                            Energy (kWh), 0x03 Power (W), 0x04 Power (kW), 0x05..0x07 not
     *                            used.
     * @param measurementDeltaMSB Measurement delta to be reported (MSB) 0-4095.
     * @param mat                 Maximum time between to subsequent actuator messages
     *                            (10-2550s).
    ...
    public void actuatorSetMeasurement(EnJConnection connection, byte deviceAddress[], byte reportMeasurement, byte resetMeasurement, byte measurementMode, byte channelId, byte measurementDeltaLSB, byte unit, byte measurementDeltaMSB, byte mat, byte mit) {
        ...
        dataByte[2] = (byte) ((reportMeasurement << 7) + (resetMeasurement << 6) + (measurementMode << 5) + channelId);
     ...
     * EnOcean Actuator.
     *
     * @param dataPayload The low level data payload to "wrap";
     * @return The wrapped payload.
     */
    public D201ActuatorStatusResponse parseActuatorStatusResponse(byte[] dataPayload) {
        // dataPayload[0] --> MSB
        ...
        return new D201ActuatorStatusResponse(powerFailure, powerFailureDetection, commandId, overCurrentSwitchOff, errorLevel, channelId, localControl, outputValue);
    }

    public D201ActuatorMeasurementResponse parseActuatorMeasurementResponse(byte[] dataPayload) {
        // dataPayload[0] --> MSB
        // dataPayload[dataPayload.length] --> LSB
    ...
    public boolean handleProfileUpdate(EEP26Telegram telegram, EnOceanDevice device) {
        ...
        if (telegram.getTelegramType() == EEP26TelegramType.VLD) {
            ...
            if (commandId == (byte) 0x04) {
                ...
                D201ActuatorStatusResponse response = parseActuatorStatusResponse(dataPayload);
                ...
                // message
                changedAttributes.addAll(updateStatusAttributes(response));

            }
            else if (commandId == (byte) 0x07) {
                // parse the actuator measurement response
                D201ActuatorMeasurementResponse response = parseActuatorMeasurementResponse(dataPayload);
                ...
                if (uom.isEnergy()) {
                    ...
                    EEPAttribute<?> energyAttribute = updateEnergyAttribute(response);
                    ...
                    EEPAttribute<?> powerAttribute = updatePowerAttribute(response);
    ...
    private ArrayList<EEPAttribute<?>> updateStatusAttributes(D201ActuatorStatusResponse response) {
        ...
        EEP26Switching switchingAttribute = (EEP26Switching) getChannelAttribute(response.getChannelId(), EEP26Switching.NAME);

        EEP26DimLevel dimLevelAttribute = (EEP26DimLevel) getChannelAttribute(response.getChannelId(), EEP26DimLevel.NAME);
        ...
        if (switchingAttribute != null || dimLevelAttribute != null) {
            ...
            if (outputLevel <= 100) {
                ...
                // update switching
                if (switchingAttribute != null && switchingAttribute.getValue() != outputLevel > 0) {
                ...
                // update dimming
                if (dimLevelAttribute != null && dimLevelAttribute.getValue() != outputLevel) {
     ...
     * (channel-specific)
     *
     * @param response The response carrying updated power data.
     */
    private EEPAttribute<?> updatePowerAttribute(D201ActuatorMeasurementResponse response) {
        ...
        EEP26PowerMeasurement powerMeasurementAttribute = (EEP26PowerMeasurement) getChannelAttribute(response.getChannelId(), EEP26PowerMeasurement.NAME);
     ...
     * (channel-specific)
     *
     * @param response The response carrying updated energy data.
     */
    private EEPAttribute<?> updateEnergyAttribute(D201ActuatorMeasurementResponse response) {
        ...
        EEP26EnergyMeasurement energyMeasurementAttribute = (EEP26EnergyMeasurement) getChannelAttribute(response.getChannelId(), EEP26EnergyMeasurement.NAME);
